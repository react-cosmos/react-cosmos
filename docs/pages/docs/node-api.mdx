...
The renderer URLs from `getFixtures()` and `cosmos.fixtures.json` are detached, so they only respond to window hooks. Load one of them and switch fixtures in-page by passing the renderer's `rendererId`, which is found in the `rendererReady` or `fixtureListUpdate` response, or in `sessionStorage['cosmosRendererId']`.
