...
Renderer URLs carry `detached=true`. A detached renderer is disconnected from the Cosmos UI and the dev server, so a Cosmos UI session on the same server can't change the fixture or its state while your visual tests run.
...
//     "rendererUrl": "http://localhost:5000/renderer.html?fixtureId=%7B%22path%22%3A%22components%2Fpages%2FError%2F__fixtures__%2Fnot-found.js%22%2C%22name%22%3Anull%7D&detached=true",
...
### Headless browser hooks

The renderer exposes two `window` globals for headless browsers. If `window.cosmosRendererResponse` is defined, the renderer calls it with every response it emits, such as `rendererReady`, `fixtureListUpdate` and `fixtureLoaded`. A runner can call `window.cosmosRendererRequest(request)` to send a request to the renderer, such as `selectFixture`, which follows the same path as requests coming from the Cosmos UI.

The renderer URLs from `getFixtures()` and `cosmos.fixtures.json` are detached, so they only respond to window hooks. Load one of them and switch fixtures in-page by passing the renderer's `rendererId`, which is found in the `rendererReady` or `fixtureListUpdate` response, or in `sessionStorage['cosmosRendererId']`. Wait for `rendererReady` before sending requests.

```ts
await page.evaluate(msg => window.cosmosRendererRequest?.(msg), {
  type: 'selectFixture',
  payload: { rendererId, fixtureId, fixtureState: {} },
});
```
