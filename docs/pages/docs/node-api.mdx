...
The renderer URLs in `cosmos.fixtures.json` carry `locked=true` and a locked renderer ignores fixture selection requests. To switch fixtures in-page, build an unlocked URL with `createRendererUrl(rendererUrl, fixtureId)` from `react-cosmos-core` and pass the renderer's `rendererId`, which is found in the `rendererReady` or `fixtureListUpdate` response, or in `sessionStorage['cosmosRendererId']`.
