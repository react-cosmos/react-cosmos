...
const mockWebpackConfig = jest.fn(() => 'MOCK_CUSTOM_WEBPACK_CONFIG');
...
jest.mock('./__fsmocks__/webpack.config', () => mockWebpackConfig);
...
it('calls webpack config function with env', () => {
  expect(mockWebpackConfig.mock.calls[0][0]).toBe(process.env.NODE_ENV);
});

it('calls webpack config function with argv', () => {
  expect(mockWebpackConfig.mock.calls[0][1]).toEqual(argv);
});
