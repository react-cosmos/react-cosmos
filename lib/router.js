...
Fresh.Router.prototype = {
  ...
  _transitionComponentContainer: function(transitionType, container) {
    ...
    // The next container will always be on top of the previous, even when
    // going backwards in history. This is OK as long as containers are faded
    // in and out when transitioned
    var $next = $(container),
        $prev = $next.prev(),
        rect = {
    ...
    Fresh.Play.start({time: 0.5, onFrame: function(ratio) {
      var translatedPositionAndScale = this._getPositionAndScaleInTransition(
            rect, transitionAnchors, transitionType, ratio);
      $prev.css(_.extend({opacity: 1 - ratio}, translatedPositionAndScale.prev));
      $next.css(_.extend({opacity: ratio}, translatedPositionAndScale.next));
      if (ratio == 1) {
        $prev.remove();
      }
    }.bind(this)});
